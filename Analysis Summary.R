...
# More details provided on the analsis in ""
...
# the naming in the line after "file =" and in the quotations "", put the location of your new file.
...
glm_vc_quasi_18_19_h <- glm(cbind(number_of_GFP_kernels, number_of_WT_kernels) ~ # using the ratio between kernels as the output
                            ...
                            family = quasibinomial(link = "logit")) # this is where the logit function is specificed
...
estimated_coeffecient = (exp(beta_list)/(1+exp(beta_list))) #estimated percentage between kernels based on beta, inverse of the logit function
...
# calculating the confidence interval using the estimated coeffecient, more details in ""
...
for (i in 2:nrow(coef(summary(glm_vc_quasi_18_19_h)))){
  allele_coeffecient = coef(summary(glm_vc_quasi_18_19_h))[i,1]
...
#This helps sort the data correctly in the table
...
for (i in 2:nrow(coef(summary(glm_sc_quasi_18_19_h)))){
  allele_coeffecient = coef(summary(glm_sc_quasi_18_19_h))[i,1]
...
for (i in 2:nrow(coef(summary(glm_seedling_only_quasi_18_19_h)))){
  allele_coeffecient = coef(summary(glm_seedling_only_quasi_18_19_h))[i,1]
...
write.table(analysis_summary_seedling_only,"clipboard",sep="\t")

#Full Table of results:

rm(analysis_summary)

analysis_summary = rbind(analysis_summary_sc,analysis_summary_seedling_only, analysis_summary_vc)
...
write.table(analysis_summary, "clipboard", sep = "\t")
